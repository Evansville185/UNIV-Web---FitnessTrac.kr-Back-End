...
const { createFakeUserWithToken, createFakeUserWithRoutinesAndActivities } = require("../helpers");
const {
	expectToBeError,
	expectNotToBeError,
...
const { getPublicRoutinesByUser, createUser, getAllRoutinesByUser } = require("../../db");
...
describe("/api/users", () => {
	describe("POST /api/users/register", () => {
		it("Creates a new user.", async () => {
			...
			// Register the user
			const response = await request(app).post("/api/users/register").send(fakeUserData);
        ...
        `,
			...
			expect(await bcrypt.compare(fakeUserData.password, hashedPassword)).toBe(true);
		});
		...
		it("Throws errors for duplicate username", async () => {
			...
			const response = await request(app).post("/api/users/register").send(secondUserData);

			expectToBeError(response.body);

			expectToHaveErrorMessage(response.body, UserTakenError(firstUser.username));
		});
		...
		it("returns error if password is less than 8 characters.", async () => {
			...
			const response = await request(app).post("/api/users/register").send(newUserShortPassword);
	...
	describe("GET /api/users/:username/routines", () => {
		it("Gets a list of public routines for a particular user.", async () => {
			...
			const { fakeUser, token } = await createFakeUserWithRoutinesAndActivities("Greg");
		...
		it("gets a list of all routines for the logged in user", async () => {
			const { fakeUser, token } = await createFakeUserWithRoutinesAndActivities("Angela");
			const response = await request(app)
